  ...
  return (
    ...
    >
      ...
      >
        ...
        {!hideMonthLabels && (
          <g className="fill-muted-foreground/40 font-medium" style={{ fontSize: fontSize * 0.9 }}>
...
export type ContributionGraphLegendProps = HTMLAttributes<HTMLDivElement> & {
  children?: ReactNode | ((props: { level: number }) => ReactNode);
};
...
export const ContributionGraphHeader = ({
  className,
  ...props
}: HTMLAttributes<HTMLDivElement>) => (
...
export const ContributionGraphTitle = ({
  className,
  ...props
...
export const ContributionGraphLabel = ({
  className,
  ...props
}: HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      "text-[9px] font-bold text-muted-foreground/60 tabular-nums uppercase tracking-wider",
      className,
    )}
    {...props}
  />
...
export const ContributionGraphActivity = ({
  className,
  ...props
}: Partial<ContributionGraphCalendarProps>) => (
  <ContributionGraphCalendar className={className} {...props}>
    {(props) => <ContributionGraphBlock {...props} />}
  </ContributionGraphCalendar>
);

export const ContributionGraphLegendItem = ({
  level,
  className,
  ...props
}: {
  ...
  const { blockSize, blockRadius } = useContributionGraph();

  return (
    <svg height={blockSize} width={blockSize} className={className} {...props}>
      <title>{`${level} contributions`}</title>
      <rect
        className={cn(
          "stroke-[1px] stroke-border",
...
export const ContributionGraphTooltip = ({
  children,
}: {
  children: ReactNode;
}) => {
